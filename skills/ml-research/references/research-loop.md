...
## Step back every handful of runs

A verdict says whether one change helped; it does not say what the runs mean together. Every five to ten runs, after a surprise, or when the best has stopped moving, stop proposing changes and read the whole log. Group the attempts by kind (which kinds of change helped, which never did) and say why for each group. Then rewrite the highlights (`findings.md` in a lab) instead of appending to them: current beliefs, each citing the runs it rests on, and constraints learned the hard way ("lr above 1e-3 diverges at this scale", "the baseline reproduces only at batch 64"). The constraints are what stop the next session from walking into the same dead end. Choose the next direction from the Decide step with that picture in view. A synthesis that changed a belief or the direction is worth a short note to the user, so they can redirect before the next batch; routine runs are not.
