...
- **2.1 (September 20, 2026).** Verification pass against the sources and a fresh-context review: added a Frame step (Nanda's Ideation, Devin's done-as-postcondition), four verdicts instead of binary keep/revert, a Never list (frozen evaluation path), an evaluator check before campaigns, report skeletons, and a fifth eval for a small question that must stay quiet. Softened per the owner: timers and statistics are rules of thumb, the phase is said only when it changes the plan, the self-review is four questions, and no log file for short work.
...
5. A quick factual question that should get a short cited answer and no machinery.

## Open

- 2.1 has not yet been run against these scenarios. Run 1, 2, and 5 first: 1 and 2 are what 1.x could not do, 5 checks that 2.x stays quiet on a small question. Evals 1 and 2 need a sandbox with a GPU; the prompts name the fixture.
